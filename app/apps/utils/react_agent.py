...
from mcp_tools.tools import collect_tool_prompts
...
def _chunk_text_for_sse(text: str, size: int = 32) -> List[str]:
    if not text:
        return []
...
class ReactAgent:
    ...
    async def run_streaming(
        ...
        try:
            listed = await self.tool_mcp.list_tools()
            openai_tools = tools_to_openai(listed)
            names_sorted = sorted(t.name for t in listed)
