...
两个用户同时问 = 两个不同 Task = 两份数据，不会串。结束时 chat_extra_reset 删掉。"""
import asyncio
import json
import traceback
...
plugin_mcp = FastMCP("fire-ops-chat-tools")
...
def chat_extra_set(initial: Dict[str, Any]) -> int:
    ...
    if task is None:
        raise RuntimeError("chat_extra_set 须在 async 协程内调用")
    tid = id(task)
    _task_extras[tid] = dict(initial)
    return tid


def chat_extra_reset(token: int) -> None:
    _task_extras.pop(token, None)


def chat_extra() -> Dict[str, Any]:
    task = asyncio.current_task()
    if task is None:
        return {}
    return _task_extras.get(id(task), {})
...
def _tool_result_to_str(result: Any) -> str:
    ...
    if isinstance(result, tuple) and result:
        return _tool_result_to_str(result[0])
...
async def run_tool(
    ...
    try:
        ...
        return _tool_result_to_str(raw)
