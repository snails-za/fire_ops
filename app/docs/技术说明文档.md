...
- 缓存和任务队列：Redis
- 异步任务：Celery
- 文档解析：LangChain loaders、openpyxl、EasyOCR
- 向量库：Qdrant / Chroma，当前默认 Qdrant
...
OCR             OCR_ENABLED、OCR_MODEL_PATH、OCR_DPI
...
解析器在 `apps/utils/document_parser.py`。

支持格式：

| 类型 | 解析方式 |
| --- | --- |
| PDF | PyMuPDFLoader、PyPDFLoader，失败后可走 OCR |
| DOC / DOCX | Docx2txtLoader |
| XLS / XLSX | 自定义 SimpleExcelLoader，基于 openpyxl |
| TXT | TextLoader |
...
- `preview` 当前按 PDF 预览响应处理，前端已限制只有 PDF 显示预览入口。
