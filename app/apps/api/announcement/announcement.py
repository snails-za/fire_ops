...
from apps.form.announcement.form import AnnouncementCreateForm, AnnouncementUpdateForm
...
async def create_announcement(
    form: AnnouncementCreateForm, user: User = Depends(get_current_user)
):
    ...
    return response(data=data.model_dump(), message="公告创建成功")
...
async def get_announcement_list(
    ...
    return response(
        data=data, total=total, total_page=total_page, message="获取公告列表成功"
    )
...
async def get_announcement_detail(announcement_id: int):
    ...
    data = await Announcement_Pydantic.from_tortoise_orm(announcement)
    return response(data=data.model_dump(), message="获取公告详情成功")


@router.put(
    "/{announcement_id}", summary="更新公告", dependencies=[Depends(get_current_user)]
)
async def update_announcement(announcement_id: int, form: AnnouncementUpdateForm):
    ...
    data = await Announcement_Pydantic.from_tortoise_orm(announcement)
    return response(data=data.model_dump(), message="公告更新成功")


@router.delete(
    "/{announcement_id}", summary="删除公告", dependencies=[Depends(get_current_user)]
)
