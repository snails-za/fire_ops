...
async def upload_image(file: UploadFile = File(...)):
    ...
    return response(
        data={"filepath": os.path.join("/", "data", "head", filename)},
        message="上传成功",
    )
...
async def user_list(username: Optional[str] = None, page: int = 1, page_size: int = 10):
    ...
    items = await User_Pydantic.from_queryset(
        query.offset((page - 1) * page_size).limit(page_size)
    )
    data = [_.model_dump() for _ in items]
    return response(
        data=data, total=total, total_page=total_page, message="获取用户列表成功！"
    )
...
async def personnel_list(
    ...
    return response(
        data=[_.model_dump() for _ in items], total=total, message="获取人员列表成功！"
    )
...
@router.get(
    "/search",
    summary="搜索联系人",
    description="按姓名、用户名或联系方式搜索可添加联系人",
    dependencies=[Depends(get_current_user)],
)
async def search_users(
    ...
    total = await query.count()
    total_page = total // page_size + (1 if total % page_size > 0 else 0)
    users = await User_Pydantic.from_queryset(
        query.offset((page - 1) * page_size).limit(page_size)
    )
    data = []
    for item in users:
        ...
        accepted = await FriendRequest.filter(
            Q(requester_id=user.id, receiver_id=target_id)
            | Q(requester_id=target_id, receiver_id=user.id),
            is_accept=True,
        ).exists()
        pending_sent = await FriendRequest.filter(
            requester_id=user.id, receiver_id=target_id, is_accept=None
        ).exists()
        pending_received = await FriendRequest.filter(
            requester_id=target_id, receiver_id=user.id, is_accept=None
        ).exists()
        user_data["is_contact"] = accepted
        user_data["pending_sent"] = pending_sent
        user_data["pending_received"] = pending_received
        user_data["can_add"] = (
            not accepted and not pending_sent and not pending_received
        )
...
async def update_user(user_id: int, user: UserUpdate):
    ...
    if (
        user.email
        and await User.filter(Q(email=user.email)).exclude(id=user_id).exists()
    ):
        return response(code=0, message="邮箱已经被注册！")
    update_data = {
        "username": user.username,
...
async def add_contact(
    user_id: int, bak: Optional[str] = None, user: User = Depends(get_current_user)
):
    if user.id == user_id:
    ...
    if await FriendRequest.filter(
        Q(requester=user, receiver=contact_user)
        | Q(requester=contact_user, receiver=user),
        is_accept=True,
    ).exists():
        return response(code=0, message="联系人已存在！")
    if await FriendRequest.filter(
        requester=contact_user, receiver=user, is_accept=None
    ).exists():
        return response(code=0, message="对方已经向您发起好友申请！请处理申请！")
    if await FriendRequest.filter(
        requester=user, receiver=contact_user, is_accept=None
    ).exists():
...
async def get_contacts(user: User = Depends(get_current_user)):
    requests = await FriendRequest.filter(
        Q(requester=user) | Q(receiver=user), is_accept=True
...
async def process_apply(id: int, process: ProcessApplyRequest):
    ...
    await FriendRequest.filter(id=id).update(
        is_accept=process.accept, updated_at=datetime.now()
    )
...
async def can_add_contact(id: int, user: User = Depends(get_current_user)):
    ...
    # 检查用户是否已添加
    if await FriendRequest.filter(
        Q(requester_id=id, receiver_id=user.id)
        | Q(requester_id=user.id, receiver_id=id),
        is_accept=True,
    ).exists():
