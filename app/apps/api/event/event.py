...
async def get_event_list(
    ...
    for event in event_rows:
        ...
        event_dict["triggered_time_display"] = event.created_at.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    return response(
        data=data, total=total, total_page=total_page, message="获取事件列表成功"
    )


@router.get(
    "/{event_id}", summary="获取事件详情", dependencies=[Depends(get_current_user)]
)
async def get_event_detail(event_id: int, user: User = Depends(get_current_user)):
    ...
    messages = (
        await EventMessage.filter(event_id=event_id).order_by("created_at").limit(50)
    )
    message_list = []
    for msg in messages:
        msg_dict = {
...
async def update_event(
    event_id: int, form: EventUpdateForm, user: User = Depends(get_current_user)
):
    """更新事件"""
...
async def send_event_message(
    event_id: int, form: EventMessageForm, user: User = Depends(get_current_user)
):
    """发送事件消息"""
    event = await Event.get_or_none(id=event_id)
...
async def get_event_messages(
    ...
    return response(
        data=data, total=total, total_page=total_page, message="获取消息列表成功"
    )


@router.get(
    "/communication/list",
    summary="获取通讯列表（进行中的事件）",
    dependencies=[Depends(get_current_user)],
)
async def get_communication_list(user: User = Depends(get_current_user)):
    """获取通讯列表：待处理/处理中的事件"""
    ...
    for event in events:
        latest_message = (
            await EventMessage.filter(event_id=event.id).order_by("-created_at").first()
        )
