...
{device_context}"""
    ...
    async def generate_answer(
        self, query: str, context_chunks: List[Dict[str, Any]], device_context: str = ""
    ) -> str:
        """
        基于检索到的文档内容生成智能回答
        ...
        try:
            ...
            for i, chunk in enumerate(context_chunks, 1):
                ...
                if chunk.get("document") and chunk.get("chunk"):
                    ...
                    print(
                        f"文档 {i}: {doc_name}, 内容长度: {len(content) if content else 0}"
                    )
                else:
                    # 使用备用格式
                    ...
                    print(
                        f"备用格式文档 {i}: {doc_name}, 内容长度: {len(content) if content else 0}"
                    )
...
内容: {content}"""
            ...
            if self.llm_available and self.chain:
                try:
                    answer = await self._llm_answer(
                        query, document_context, device_context
                    )
    ...
    async def generate_answer_stream(
        ...
        try:
            ...
            for i, chunk in enumerate(context_chunks, 1):
                # 直接使用数据结构
                doc_name = (
                    chunk["document"].original_filename or chunk["document"].filename
                )
                content = chunk["chunk"].content
                print(
                    f"流式文档 {i}: {doc_name}, 内容长度: {len(content) if content else 0}"
                )
            ...
            if self.llm_available and self.chain:
                try:
                    print("使用LLM流式生成回答")
                    async for chunk in self._llm_answer_stream(
                        query, document_context, device_context
                    ):
                        yield chunk
                except Exception as e:
                    ...
                    simple_answer = self._simple_answer(
                        query, context_chunks, device_context
                    )
                    yield simple_answer
            else:
    ...
    async def _llm_answer_stream(
        self, query: str, document_context: str, device_context: str
    ):
        """
    ...
    def _simple_answer(
        ...
        try:
            ...
            print(
                f"简单回答模式 - 文档块数量: {len(context_chunks) if context_chunks else 0}"
            )
            ...
            if context_chunks:
                ...
                document_name = (
                    best_chunk["document"].original_filename
                    or best_chunk["document"].filename
                )
                content = best_chunk["chunk"].content
                answer_parts.append(
                    f"📄 **基于文档《{document_name}》中的相关内容：**\n\n{content}"
                )
            else:
                print("没有文档块，跳过文档信息")
