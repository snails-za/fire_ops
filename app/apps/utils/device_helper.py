...
async def get_all_devices_by_permission(
    ...
    try:
        ...
        if is_admin:
            ...
            status_distribution = {}
            for status in ["正常", "离线", "异常", "维修中"]:
                count = await Device.filter(
                    created_by_user_id=user_id, status=status
                ).count()
                status_distribution[status] = count

            # 获取设备列表（限制数量）
            devices_query = (
                Device.filter(created_by_user_id=user_id)
                .order_by("-id")
                .limit(MAX_DEVICES_FOR_LLM)
            )
            ...
            device_list.append(
                {
                    ...
                    "install_date": str(device.install_date)
        ...
        if result["is_partial"]:
            print(
                f"⚠️ 注意: 设备总数 {total_count} 台，仅返回前 {MAX_DEVICES_FOR_LLM} 台详情，但统计信息完整"
            )
...
async def get_all_devices_summary(
    ...
    try:
        ...
        print(f"获取设备统计失败: {e}")
        return {"total": 0, "by_status": {}, "devices": []}
...
def format_device_context(device_data: Dict[str, Any]) -> str:
    ...
    print(
        f"格式化设备上下文: 总数={total}, 详情数量={len(devices)}, 是否部分数据={is_partial}"
    )
