...
class Event(BaseModel):
    ...
    title = fields.CharField(
        max_length=200, description="事件标题，如：3号楼・烟感告警 (A区 2F)"
    )
    level = fields.CharField(
        max_length=20,
        default="medium",
        description="事件等级：高(high)、中(medium)、低(low)",
    )
    status = fields.CharField(
        max_length=20,
        default="wait",
        description="事件状态：待处理(wait)、处理中(processing)、已关闭(closed)",
    )
    ...
    location = fields.CharField(
        max_length=200, null=True, description="位置信息，如：3号楼A区2F"
    )
...
class EventMessage(BaseModel):
    ...
    user = fields.ForeignKeyField(
        ...
        on_delete=fields.SET_NULL,
        null=True,
        description="发送用户（null表示系统消息）",
    )
    username = fields.CharField(
        max_length=50, null=True, description="用户名（冗余字段）"
    )
    user_role = fields.CharField(
        max_length=20, null=True, description="用户角色（冗余字段）"
    )
    ...
    message_type = fields.CharField(
        max_length=20,
        default="user",
        description="消息类型：user(用户消息)、system(系统消息)",
    )
