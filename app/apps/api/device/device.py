...
async def upload_image(file: UploadFile = File(...)):
    ...
    return response(
        data={"filepath": os.path.join("/", "data", "device", filename)},
        message="上传成功",
    )
...
async def create_device(device: DeviceIn, user: User = Depends(get_current_user)):
    ...
    if is_privileged(user):
        ...
        exists = await Device.filter(
            name=device.name, created_by_user_id=user.id
        ).exists()
    ...
    if device_status and device_status not in valid_statuses:
        return response(
            code=400, message=f"设备状态无效，只允许：{', '.join(valid_statuses)}"
        )
...
async def update_device(
    device_id: int, device: DeviceUpdate, user: User = Depends(get_current_user)
):
    """
    更新设备信息
    ...
    if new_status == "正常" and old_status in ["告警", "异常", "离线"]:
        ...
        ongoing_events = await Event.filter(
            device_id=device_obj.id, status__in=["wait", "processing"]
        )
...
@router.get(
    "/list",
    response_model=list[Device_Pydantic],
    summary="设备列表",
    description="获取设备列表",
    dependencies=[Depends(get_current_user)],
)
async def device_list(
    ...
    return response(
        data=data, total=total, total_page=total_page, message="获取设备列表成功"
    )
